...
| `/api/get-subs/param`             | GET   | Расчет общей стоимости подписок по параметрам |
| `/api/get-subs/user-id/one`       | GET    | Получение конкретной подписки по её ID        |
| `/api/get-subs/user-id/all`       | GET    | Получение всех подписок пользователя          |
